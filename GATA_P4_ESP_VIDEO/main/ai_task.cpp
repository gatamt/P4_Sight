...
static void ai_inference_task(void *arg) {
  ...
  while (true) {
    ...
    /* Startup diagnostic retained from the format investigation. The active
     * O_UYY_E_VYY frame has no separate luma and chroma planes; this samples
     * fixed offsets only to make unexpected layout changes visible in logs. */
    if (sweep_count < 2) {
      ...
      /* Compare two historical probe offsets used during layout validation. */
      ESP_LOGI(
          TAG,
