...
/** Latest captured YUV420 frame metadata, published by the capture loop.
 *  The pointer is non-owning and lock-free: after the capture buffer is
 *  requeued, the AI task may observe contents updated by subsequent DMA. */
...
void app_main(void) {
  ...
  while (true) {
    ...
    /* Publish non-owning capture-buffer metadata to the asynchronous AI task. */
