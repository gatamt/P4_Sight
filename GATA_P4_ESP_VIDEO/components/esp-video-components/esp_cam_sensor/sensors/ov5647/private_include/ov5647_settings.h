...
static const ov5647_reginfo_t ov5647_input_24M_MIPI_2lane_raw8_800x640_15fps[] = {
    // ---------- PLL and format (retain the 50 fps clock settings) ----------
    {0x3034, OV5647_8BIT_MODE},                   // RAW8
    {0x3035, 0x41},                               // sysclk div
    {0x3036, ((OV5647_IDI_CLOCK_RATE_800x640_15FPS * 8 * 4) / 25000000)}, // PLL mult (≈100 MHz)
    {0x303c, 0x11},
    {0x3106, 0xf5},
    ...
    // ---------- Exposure and gain ----------
    // Use automatic exposure and gain to avoid underexposure at 15 fps.
    {0x3503, 0x00},        // AEC/AGC ON
    // These registers do not need to be set explicitly here.
    ...
    // ---------- janela/crop para 800x640 ----------
    {0x3800, (500 >> 8) & 0x0F},       // X start
    {0x3801,  500       & 0xFF},
    {0x3802, (0   >> 8) & 0x07},       // Y start
    {0x3803,  0         & 0xFF},
    {0x3804, ((2624 - 1) >> 8) & 0x0F},// X end
    {0x3805,  (2624 - 1)       & 0xFF},
    {0x3806, ((1954 - 1) >> 8) & 0x07},// Y end
    {0x3807,  (1954 - 1)       & 0xFF},
    {0x3808, (800 >> 8) & 0x0F},       // out width
    {0x3809,  800       & 0xFF},
    {0x380a, (640 >> 8) & 0x7F},       // out height
    {0x380b,  640       & 0xFF},
    {0x3810, (8   >> 8) & 0x0F},       // hoffset
    {0x3811,  8         & 0xFF},
    {0x3812, (0   >> 8) & 0x07},       // voffset
    {0x3813,  0         & 0xFF},

    // ---------- ISP and analog settings retained from stable presets ----------
    {0x3630, 0x2e},
    {0x3632, 0xe2},
    {0x3633, 0x23},
    {0x3634, 0x44},
    ...
    // ---------- AEC tables retained for compatibility ----------
    {0x3a08, 0x01},
    {0x3a09, 0x27},
    {0x3a0a, 0x00},
    {0x3a0b, 0xf6},
    ...
    {0x4004, 0x02},
    {0x4000, 0x09},
    // MIPI timing coerente com PCLK≈100 MHz
    {0x4837, (1000000000 / (OV5647_IDI_CLOCK_RATE_800x640_15FPS / 4))},
