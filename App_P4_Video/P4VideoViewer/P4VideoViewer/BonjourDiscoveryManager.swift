...
/// Discovers _gatavideo._udp services on the local network via Bonjour/mDNS.
...
final class BonjourDiscoveryManager: @unchecked Sendable {
  private let logger = Logger(subsystem: "com.gata.p4videoviewer", category: "discovery")
  ...
  func startBrowsing() {
    ...
    let descriptor = NWBrowser.Descriptor.bonjour(type: "_gatavideo._udp", domain: nil)
    ...
    logger.info("Started browsing for _gatavideo._udp")
  }
...
final class HotspotProbeDiscoveryManager: @unchecked Sendable {
  private let logger = Logger(subsystem: "com.gata.p4videoviewer", category: "probe")
  private let queue = DispatchQueue(label: "com.gata.p4videoviewer.probe", qos: .userInitiated)
